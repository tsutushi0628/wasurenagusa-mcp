...
記憶の検索精度を3つのアプローチで改善する。データの削除や忘却は一切行わない。全記憶を保持したまま、「取り出しの優先順位」のみを最適化する設計。

1. **保存時のタグ拡張+重み付け**: `memory_save` 時に Gemini LLM がタグを自動生成（7〜15個）。各タグに重み（0.0〜1.0）を付与 — 具体的な事実値（ポート番号、API名）は高重み、汎用カテゴリ（API、設定）は低重み
2. **テーマ変動時の再タグ付け**: 新しいテーマが検出されると、バックグラウンドワーカーが関連する過去エントリのタグを自動更新
3. **複合スコアリング**: freshness（鮮度）× タグ重み × アクセス頻度を組み合わせた検索結果ランキング。最近の記憶、重要なタグを持つ記憶、頻繁にアクセスされる記憶が上位に浮上する
