...
- **Language**: TypeScript 5.x + Node.js 20.x (ES2022)
...
├── config.ts             # 設定管理
├── cli/
│   ├── context.ts        # SessionStart Hook用CLI
│   ├── analyze.ts        # Stop Hook用CLI
│   ├── spec-update.ts    # Spec自動更新CLI
│   └── transcript-reader.ts # トランスクリプトJSONLパーサー
├── tools/
│   ├── index.ts          # エクスポート
│   ├── getContext.ts     # memory_get_context
│   ├── save.ts           # memory_save
│   ├── search.ts         # memory_search
│   ├── getDetail.ts      # memory_get_detail
│   └── delete.ts         # memory_delete
├── storage/
│   ├── index.ts          # インターフェース
│   ├── markdown.ts       # Markdown読み書き
│   ├── parser.ts         # Markdownパーサー
│   └── formatter.ts      # Markdownフォーマッター
├── analyzer/
│   ├── index.ts          # エクスポート
│   ├── gemini.ts         # Gemini API連携
│   ├── prompt-loader.ts  # プロンプトファイル読み込み
│   └── conversation-meta.ts # 会話メタ情報（諦め検知用）
├── consolidator/
│   ├── index.ts          # エクスポート
│   ├── dont-consolidator.ts # dontエントリ統合
│   ├── staleness.ts      # 統合キャッシュ鮮度チェック
│   └── formatter.ts      # 統合結果フォーマッター
├── scheduler/
│   ├── change-logger.ts  # 変更ログ記録
│   ├── task-queue.ts     # タスクキュー管理
│   ├── executor.ts       # Claude CLI実行
│   └── prompt-builder.ts # プロンプト組み立て
└── utils/
    └── projectRoot.ts    # .git探索
```
...
- **Component Isolation**: CLI/Tools/Storage/Analyzer/Consolidator/Schedulerの6層分離
...
- **Dependencies**: GeminiAnalyzer, MarkdownStorage, findProjectRoot
- **REQ対応**: REQ-2, REQ-3
...
- **Purpose**: 多数のdontエントリをGeminiで少数の行動原則に統合（L3圧縮）
- **Interfaces**: `consolidate(entries: MemoryEntry[]) => ConsolidatedDont | null`
- **Dependencies**: @google/generative-ai, analyzer/prompt-loader.ts
