...
- 検査項目：recall（正解実在クラスの recall@5 が、同一ゴールデンセットを旧検索で実行して再測定した基準値〔タスク2.3の記録〕を超えること。recall@1 と recall@10 も記録。母集団の異なる過去計測値を基準に使わない）、correct-zero（「正しくゼロ件」クラス全問で0件）、self-search（PT-04：全生存エントリの自己検索性100%。本文3文字未満はFTS最短長制約により対象外とし除外件数を出力。失敗は件数のみ出力）、read-no-side-effect（検索実行の前後でDB内容が不変。R-B2 AC3の明示検査）、fallback-counters（各段の発火計数が出力される）、shadow-report（新旧並走比較レポートが存在し、新系が主要指標で旧系以上）
  - **【NOTE：recall@5 は order-blind／係数変更時は順序指標の非退行も必須】** recall@5 は「上位5件への集合帰属」だけを見る指標であり、**上位5件内での順序退行を検出できない**（順序に対して盲目）。本設計は time-decay を主ランキング軸として順位を決定すると規定している（`finalScore = rrfScore × 0.5^(ageDays/H)`。順位＝成果物）以上、その主軸に関わる係数——減衰半減期(H)・RRF定数(RRF_K)・段重み・候補プール——を変更する場合、recall@5 非退行だけでは足りない。**eval-order ハーネスの A/B 突合で、順序指標（rank-precision@expectedRankMax・MRR）の非退行も併せて示すこと**を追加要件とする。順序指標は当面「記録層」（recall@1/@10 と同格の measured）であり本ゲートの合否条件（recall@5 > 基準値）は不変だが、係数変更の go/no-go では順序指標の非退行が必須条件となる。
