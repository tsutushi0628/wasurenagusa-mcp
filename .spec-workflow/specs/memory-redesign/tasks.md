...
- [x] 1.1 多言語埋め込みモデル候補の実在確認スパイク（コミット b2a8c3a・2026-07-10）
...
- [x] 1.2 日本語トークナイザ先行スパイク（クエリ側整合の再計測）（コミット 7d73a92・2026-07-10）
...
- [x] 2.1 ゴールデンセットの採取と凍結（検証役・2026-07-10完了）
  - 完了注記: 実操作ログ12,113検索から52問採取（ヒット期待37・正しくゼロ件15、4クラス全種）。正解ラベルは検索非使用のLIKEプローブ＋意味読解で付与し、expectedIds全77件のactive存在を機械検算。sha256記録・凍結。統計と証跡所在は Implementation Logs/task-2.1-golden-set.md（クエリ本文はGit外のみ）
