...
│   ├── storage/          # 永続化層。sqlite.ts が正本（v2）。markdown.ts はv1遺物、migration.ts はv1→v2移行
...
7. **作業ツリーの未コミットWIP**：着手時点の作業ツリーには、予測誤差ループ（死機能）のWIP（追跡ファイルへの変更436行と未追跡4ファイル）が残置されている。コミット済みHEADにこのコードは存在しない。Phase 0のタスク0.0で退避パッチ化のうえ破棄し、クリーンなHEADから着手する。WIP適用後の状態を前提にした実装や行番号引用をしない
