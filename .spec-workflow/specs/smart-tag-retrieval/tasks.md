...
- [ ] 1. WeightedTag型定義とタグパースユーティリティのテスト作成
  - File: `src/vector/__tests__/weighted-tag.test.ts`
...
- [ ] 2. WeightedTag型定義とパース/フォーマットユーティリティの実装
...
- [ ] 3. parser.tsのタグパースを重み付き対応に拡張するテスト作成
  - File: `src/storage/__tests__/parser-weighted-tag.test.ts`
...
- [ ] 4. formatter.tsのタグ出力を重み付き対応にするテスト作成と実装
  - File: `src/storage/__tests__/formatter-weighted-tag.test.ts`, `src/storage/formatter.ts`
...
- [ ] 5. SearchScorer freshness計算のテスト作成
  - File: `src/vector/__tests__/search-scorer.test.ts`
...
- [ ] 6. SearchScorer tagWeightScore計算のテスト作成
  - File: `src/vector/__tests__/search-scorer.test.ts`（Task 5に追記）
...
- [ ] 7. SearchScorer accessBoost計算のテスト作成
  - File: `src/vector/__tests__/search-scorer.test.ts`（Task 5に追記）
...
- [ ] 8. SearchScorer.score() 複合スコアのテスト作成
  - File: `src/vector/__tests__/search-scorer.test.ts`（Task 5に追記）
...
- [ ] 10. TagEnricherのテスト作成
  - File: `src/vector/__tests__/tag-enricher.test.ts`
...
- [ ] 13. save.tsタグ拡張統合のテスト作成
  - File: `src/tools/__tests__/save-tag-enrichment.test.ts`
...
- [ ] 15. search.tsスコアリング統合のテスト作成
  - File: `src/tools/__tests__/search-scoring.test.ts`
...
- [ ] 16. VectorStoreにlastAccessedAt取得メソッド追加
  ...
  - _Prompt: Role: TypeScript Developer | Task: Add getEntryMetadata(ids: string[]) to VectorStore. Returns Map<id, {lastAccessedAt, accessCount}> for the given IDs. Reads from existing VectorEntry fields (no schema change needed). Add test in src/vector/__tests__/vector-store-metadata.test.ts. | Restrictions: Read-only method. Don't modify existing methods. | Success: Metadata retrieval works for existing, missing, and mixed ID sets_

- [ ] 17. search.tsにSearchScorer統合を実装
...
- [ ] 18. memory_search/memory_get_detailでのlastAccessedAt更新テスト
  - File: `src/vector/__tests__/access-tracking.test.ts`
...
- [ ] 19. getDetail.tsにlastAccessedAt更新を追加
...
- [ ] 20. ThemeRegistryのテスト作成と実装
  - File: `src/vector/__tests__/theme-registry.test.ts`, `src/vector/theme-registry.ts`
...
- [ ] 21. RetagWorkerのテスト作成
  - File: `src/cli/__tests__/retag-worker.test.ts`
...
- [ ] 23. save.tsからRetagWorkerのspawn統合
  - File: `src/tools/save.ts`
...
- [ ] 24. 後方互換性テスト（重みなしタグの読み込み）
  - File: `src/vector/__tests__/backward-compat.test.ts`
...
- [ ] 25. memory_save -> memory_search E2E検証テスト
  - File: `src/__tests__/e2e-smart-tag-retrieval.test.ts`
